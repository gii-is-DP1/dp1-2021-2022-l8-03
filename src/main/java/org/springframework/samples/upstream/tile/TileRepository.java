...
public interface TileRepository extends CrudRepository<Tile, Integer>{
	...
	@Query("SELECT tile FROM Tile tile WHERE tile.salmonEggs=:salmoneggs AND tile.round.id=:round_id")
	public Tile findTileBySalmonEggs(@Param("salmoneggs") int salmoneggs, @Param("round_id") int round_id);
}